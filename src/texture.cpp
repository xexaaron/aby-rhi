...
namespace aby::rhi {
	...
	auto Texture::create(uint32_t width, uint32_t height, uint8_t channels, std::vector<uint8_t>&& bytes) -> ResourcePtr<Texture, EResource::texture> {
		aby_rhi_assert(Context::get().job_sys(), "context was not initialized");
		auto backend = Context::get().renderer_backend();
		auto* jobs   = Context::get().job_sys();
		auto& texs   = Context::get().textures();
		switch (backend) {
			case ERenderer::vulkan: {
				...
				jobs->add_job(EJobPriority::high, [resource, width, height, channels, bytes = std::move(bytes)]() {
					auto& texs    = Context::get().textures();
					auto& plugins = Context::get().plugins();
